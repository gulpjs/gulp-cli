...
function printTreeList(lines, spacer, lineInfos) {
  lines.forEach(function(branch, index) {
    ...
    var line = color.treeBranch(branch);

    if (info.type === 'top') {
      line += color.task(info.name);
      if (info.desc.length > 0) {
        line += spacer(index) + color.taskDesc(info.desc);
      }
    } else if (info.type === 'option') {
      line += color.flag(info.name);
      if (info.desc.length > 0) {
        line += spacer(index) + color.flagDesc('…' + info.desc);
      }
    } else { // If (info.type === 'child') {
      line += color.childTask(info.name);
    }

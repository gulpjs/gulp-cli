...
function printTreeList(lines, spacer, lineInfos) {
  lines.forEach(function(branch, index) {
    ...
    if (info.type === 'top') {
      ...
      log.info(msgs.tasks.topTask, branch, info.name, Boolean(desc), spaces, desc);
      ...
      log.info(msgs.tasks.option, branch, info.name, Boolean(desc), spaces, desc);

    } else { // If (info.type === 'child') {
      log.info(msgs.tasks.childTask, branch, info.name);
    }
  });
}

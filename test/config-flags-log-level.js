...
describe('config: flag.logLevel', function() {
  describe('log level 3 by default', function() {
    ...
    it('Should output warn log', function(done) {
      var packageJsonPath = path.resolve(__dirname, 'fixtures/packages', 'invalid-package.json');
    ...
    it('Should output info log', function(done) {
      var packageJsonPath = path.resolve(__dirname, 'fixtures/packages', 'valid-package.json');
  ...
  describe('log level 1 by config `flags.logLevel`', function() {
    var gulp = runner({ verbose: false }).basedir(path.join(__dirname, 'fixtures/config/flags/logLevel/L')).gulp;

    it('Should output error log', function(done) {
      gulp('--gulpfile x').run(function(err, stdout, stderr) {
        expect(err).toNotEqual(null);
        expect(stdout).toEqual('');
        expect(eraseTime(stderr)).toEqual('No gulpfile found\n');
        done();
    ...
    it('Should output warn log', function(done) {
      ...
      gulp('--verify', packageJsonPath).run(function(err, stdout, stderr) {
        expect(err).toNotEqual(null);
        expect(stdout).toEqual('');
        expect(stderr).toEqual('');
        done();
  ...
  describe('log level 2 by config `flags.logLevel`', function() {
    var gulp = runner({ verbose: false }).basedir(path.join(__dirname, 'fixtures/config/flags/logLevel/LL')).gulp;
  ...
  describe('log level 3 by config `flags.logLevel`', function() {
    var gulp = runner({ verbose: false }).basedir(path.join(__dirname, 'fixtures/config/flags/logLevel/LLL')).gulp;
